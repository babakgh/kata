package switch_imp_test

import (
	"testing"

	"github.com/babakgh/kata/finite-state-machine/go/pkg/core/switch_imp"
	. "github.com/onsi/ginkgo/v2"
	. "github.com/onsi/gomega"
)

func TestSwitchImp(t *testing.T) {
	RegisterFailHandler(Fail)
	RunSpecs(t, "Switch Implementation Suite")
}
...
var _ = Describe("FSM", func() {
	var (
		...
		fsm       *switch_imp.TurnstileFSM
	)

	BeforeEach(func() {
		turnstile = NewFakeTurnstile()
		fsm = switch_imp.NewTurnstileFSM(turnstile)
	})
	...
	Describe("handling coins and passes", func() {
		Context("when the state is Locked", func() {
			It("should unlock when a coin is inserted", func() {
				fsm.HandleEvent(switch_imp.Coin)
				Expect(fsm.CurrentState).To(Equal(switch_imp.Unlocked))
			...
			It("should remain locked and raise an alarm when a pass is used", func() {
				fsm.HandleEvent(switch_imp.Pass)
				Expect(fsm.CurrentState).To(Equal(switch_imp.Locked))
		...
		Context("when the state is Unlocked", func() {
			BeforeEach(func() {
				fsm.HandleEvent(switch_imp.Coin) // Unlock the FSM
			})
